...
class FairCorelsClassifier:
    ...
    mode: int optional (default=3)
        ...
        // Sample-robust modes:
        5: Sample-Robust, weighted sum (f_obj-beta*sample_robustness)
        6: Sample-Robust, epsilon-constrained method

    min_sample_robustness: double optional (default = -1.0 (so that the constraint is not active))
        When mode == 6, minimum acceptable value for the sample-robustness score of fairness
    ...
    def __init__(self, c=0.01, n_iter=10000, map_type="prefix", policy="lower_bound",
                 ...
                 bfs_mode=0, random_state=42, min_sample_robustness=-1.0, sample_robustness_auditor_object=None):
        self.c = c
        self.n_iter = n_iter
        ...
        self.min_sample_robustness = min_sample_robustness
        if(maj_vect.size == 0):
        ...
        self.sample_robustness_auditor_object = sample_robustness_auditor_object
        self.mode = mode
    ...
    def fit(self, X, y, features=[], prediction_name="prediction", performRestarts=0, initNBNodes=1000, geomRReason=1.5, max_evals=1000000000, time_limit = None):
        ...
        if not isinstance(self.min_sample_robustness, float):
            raise TypeError("Minimum acceptable fairness sample-robustness must be a float, got: " + str(type(self.min_sample_robustness)))
        # Todo check mode, .... (?)
        if self.sample_robustness_auditor_object is None:
            self.sample_robustness_auditor_object = SampleRobustnessAuditor(1)
        else:
            print("User provided existing sample-robustness auditor")
            print("Memo info is :")
            self.sample_robustness_auditor_object.print_memo_statistics()
        if(self.min_pos != -2):
        ...
        fr = fit_wrap_begin(samples.astype(np.uint8, copy=False),
                             ...
                             self.sample_robustness_auditor_object)
        
        if fr:
            early = False
            try:
                if time_limit is None: 
                    while fit_wrap_loop(self.n_iter, self.beta, self.fairness, self.mode, self.useUnfairnessLB, self.epsilon, self.kbest, performRestarts, initNBNodes, geomRReason, self.min_sample_robustness):
                        pass
                else:
                    import time
                    start = time.clock()
