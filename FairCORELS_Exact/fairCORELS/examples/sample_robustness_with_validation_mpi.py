...
for train_index, test_index in kf.split(X):
    ...
    X_train_train, X_train_val, y_train_train, y_train_val = train_test_split(X_train, y_train, test_size=0.25, random_state=43, shuffle=True)
    folds.append([X_train_train, y_train_train, X_train_val, y_train_val, X_test, y_test])
...
def oneFold(foldIndex, X_fold_data): # This part could be multithreaded for better performance
    X_train, y_train, X_val, y_val, X_test, y_test = X_fold_data
    ...
    # - Validation set
    sensVect_val =  X_val[:,sensitive_attr_column]
    unSensVect_val =  X_val[:,unsensitive_attr_column] 
    X_val_unprotected = X_val[:,2:]
    ...
    '''print("Training:")
    print("sens vect: ", sensVect_train.shape)
    print("unsens vect: ", unSensVect_train.shape)
    print("unprotected feats vect: ", X_train_unprotected.shape)
    print("labels vect: ", y_train.shape)

    print("Validation:")
    print("sens vect: ", sensVect_val.shape)
    print("unsens vect: ", unSensVect_val.shape)
    print("unprotected feats vect: ", X_val_unprotected.shape)
    print("labels vect: ", y_val.shape)

    print("Test:")
    print("sens vect: ", sensVect_test.shape)
    print("unsens vect: ", unSensVect_test.shape)
    print("unprotected feats vect: ", X_test_unprotected.shape)
    print("labels vect: ", y_test.shape)'''
    ...
    accVal = clf.score(X_val_unprotected, y_val)
    ...
    val_preds = clf.predict(X_val_unprotected)
    unfVal = compute_unfairness(sensVect_val, unSensVect_val, y_val, val_preds)
    ...
    #exit()
    sys.stdout.flush()
    return [foldIndex, accTraining, unfTraining, objF, accTest, unfTest, exploredBeforeBest, cacheSizeAtExit, length, fairness_sample_robustness.jaccard_dist, clf.rl_, accVal, unfVal]
...
found = True
while found: 
    fileName = './results_with_validation/sample_robust_frontier_with-validation_%s_%d_faircorels_eps%f_metric%d_LB%d.csv' %(dataset, frontier_id, epsilon, fairnessMetric, useLB)
    try:
...
if check_triviality_reached_all_folds(current_sample_robustness) or (check_all_ok(foldOK_Epsilon, foldOK_Train)):
    mustStop = True
...
while not mustStop:
    ...
    if rank == 0:
        ...
        resPerFold = dict()
        for aRes in ret:
            resPerFold[aRes[0]] = [aRes[1], aRes[2], aRes[3], aRes[4], aRes[5], aRes[6], aRes[7], aRes[8], aRes[9], aRes[10], aRes[11], aRes[12]]
            current_sample_robustness[aRes[0]] = aRes[9]
        val_unf = np.average([resPerFold[index][11] for index in range(5)])
        val_unf_list = [resPerFold[index][11] for index in range(5)]
        ...
        print("=========> Validation Accuracy (average)= ", np.average([resPerFold[index][10] for index in range(5)]))
        print("=========> Validation Unfairness (average)= ", val_unf)
        print("=========> Training Sample Robustness (average)= ", np.average([resPerFold[index][8] for index in range(5)]))
        sys.stdout.flush()
   
        with open('./results_with_validation/sample_robust_frontier_with-validation_%s_%d_faircorels_eps%f_metric%d_LB%d.csv' %(dataset, frontier_id, epsilon, fairnessMetric, useLB), mode='w') as csv_file:
            csv_writer = csv.writer(csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            csv_writer.writerow(['Fold#', 'Training accuracy', 'Training Unfairness(%d)' %fairnessMetric, 'Training objective function', 'Training Sample-Robustness', 'Test accuracy', 'Test unfairness', '#Nodes explored for best solution', 'Cache size for best solution', 'Average length', 'RL', 'Validation accuracy', 'Validation unfairness'])#, 'Fairness STD', 'Accuracy STD'])
            for index in range(5):
                csv_writer.writerow([index, resPerFold[index][0], resPerFold[index][1], resPerFold[index][2], resPerFold[index][8], resPerFold[index][3], resPerFold[index][4], resPerFold[index][5], resPerFold[index][6], resPerFold[index][7], resPerFold[index][9], resPerFold[index][10], resPerFold[index][11]])#,fairnessSTD_list_test_tot[i][fairnessMetric-1], accuracySTD_list_test_tot[i]])
            csv_writer.writerow(['Average', np.average(accuracyT), np.average(unfairnessT), np.average(objective_functions), np.average([resPerFold[index][8] for index in range(5)]), np.average(accuracy), np.average(unfairness), np.average([resPerFold[index][5] for index in range(5)]), np.average([resPerFold[index][6] for index in range(5)]), np.average([resPerFold[index][7] for index in range(5)]), '', np.average([resPerFold[index][10] for index in range(5)]), np.average([resPerFold[index][11] for index in range(5)])])#,fairnessSTD_list_test_tot[i][fairnessMetric-1], accuracySTD_list_test_tot[i]])

        for ff in range(5):
            if val_unf_list[ff] <= 1.0 - epsilon:
                foldOK_Epsilon[ff] = True
            if val_unf_list[ff] <= unfairnessT[ff]:
                foldOK_Train[ff] = True
        if check_triviality_reached_all_folds(current_sample_robustness) or (check_all_ok(foldOK_Epsilon, foldOK_Train)):
            mustStop = True
        '''if check_triviality_reached_all_folds(current_sample_robustness) or (val_unf <= 1.0 - epsilon and val_unf <= np.average(unfairnessT)):
            mustStop = True
        else:
            mustStop = False'''
