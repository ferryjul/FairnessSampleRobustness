...
#include <chrono> 
#include "../../../_corels.h" // for sample robustness auditing
#include <fstream> // TODO delete when not used anymore (useful only for debugging sample robustness scores)
...
// Global variables for sample robustness auditing
double max_robustness = -1;
double min_robustness = 1;
double max_robustness_not_one = -1;
std::vector<double> jaccard_distances;// = new std::vector<double>();
...
void evaluate_children(CacheTree* tree, 
    ...
    for (i = 1; i < nrules; i++) {
        ...
        // Deleted for this sample-robust version of FairCORELS
        ...
        // Quantify sample robustness
        double fairness_sample_robustness = 0.0;
        if((mode == 5 && beta != 0.0)){ // If fairness sample robustness value is not used later, we save time by not computing it!
            if((1-unfairness) > min_fairness_acceptable){
                ...
                if ((nb_sp_plus + nb_sp_minus) != (TPp + FPp + TNp + FNp)){
                    std::cout << "Error: (nb_sp_plus + nb_sp_minus) = " << (nb_sp_plus + nb_sp_minus) << " != " << (TPp + FPp + TNp + FNp) << " = (TPp + FPp + TNp + FNp)" << std::endl;
                    exit(-1);
                }
                if ((nb_su_plus + nb_su_minus) != (TPu + FPu + TNu + FNu)){
                    std::cout << "Error: (nb_su_plus + nb_su_minus) = " << (nb_su_plus + nb_su_minus) << " != " << (TPu + FPu + TNu + FNu) << " = (TPu + FPu + TNu + FNu)" << std::endl;
                    exit(-1);
                }
                switch (fairness)
                {
                    case 1:
                        fairness_sample_robustness = compute_sample_robustness(TPp + FPp, TPu + FPu, TPp + FPp + TNp + FNp, TPu + FPu + TNu + FNu, 1.0 - min_fairness_acceptable, 0, 0, 1);
                        //std::cout << "Statistical Parity: Sample robustness score = " << fairness_sample_robustness << std::endl;
                        break;
                    case 2:
                        std::cout << "Exact fairness Sample Robustness not supported for Predictive Parity!" << std::endl;
                        exit(-1);
                        break;
                    case 3:
                        fairness_sample_robustness = compute_sample_robustness(FPp, FPu, FPp + TNp, FPu + TNu, 1.0 - min_fairness_acceptable, 0, 0, 1);
                        //std::cout << "Statistical Parity: Sample robustness score = " << fairness_sample_robustness << std::endl;
                        break;
                    case 4:
                        fairness_sample_robustness = compute_sample_robustness(FNp, FNu, TPp + FNp, TPu + FNu, 1.0 - min_fairness_acceptable, 0, 0, 1);
                        //std::cout << "Statistical Parity: Sample robustness score = " << fairness_sample_robustness << std::endl;
                        break;
                    case 5:
                        {
                        //std::cout << "Exact fairness Sample Robustness not supported for EOdds!" << std::endl;
                        //std::cout << "Note that EOdds can be implemented (future work)" << std::endl;
                        // A more efficient implementation could be performed, including conjunction directly within the model!
                        double pe_fairness_sample_robustness = compute_sample_robustness(FPp, FPu, FPp + TNp, FPu + TNu, 1.0 - min_fairness_acceptable, 0, 0, 1);
                        double eo_fairness_sample_robustness = compute_sample_robustness(FNp, FNu, TPp + FNp, TPu + FNu, 1.0 - min_fairness_acceptable, 0, 0, 1);
                        fairness_sample_robustness = fmin(pe_fairness_sample_robustness, eo_fairness_sample_robustness);
                        break;
                        }
                    case 6:
                        std::cout << "Exact fairness Sample Robustness not supported for CUAE!" << std::endl;
                        exit(-1);
                        break;
                    default:
                        break;
                }
                //jaccard_distances.push_back(fairness_sample_robustness);
                // Update min and max sample robustness
                if(fairness_sample_robustness > 0.0){
                    if(fairness_sample_robustness < min_robustness){
                        min_robustness = fairness_sample_robustness;
                    }
                    if(fairness_sample_robustness > max_robustness){
                        max_robustness = fairness_sample_robustness;
                    }
                    if(fairness_sample_robustness > max_robustness_not_one && fairness_sample_robustness < 1.0){
                        max_robustness_not_one = fairness_sample_robustness;
                    }
                }
            }
        }
        ...
        if(mode == 2) { // Max fairness
        ...
        } else if(mode == 3 || mode == 6) { // Espilon Constraint mode
            double misc = (double)(num_not_captured - default_correct) / nsamples;
            objective =  misc + lower_bound;
        } else if(mode == 4) { // Max accuracy 
            double misc = (double)(num_not_captured - default_correct) / nsamples;
            objective =  misc + lower_bound;
        } else if(mode == 1) { // Regularized mode
            double misc = (double)(num_not_captured - default_correct) / nsamples;
        ...
        } else if(mode == 5) { // Sample-robust mode
            double misc = (double)(num_not_captured - default_correct) / nsamples;
            /* Distance-to-a-reference objective function */
            /*double unfairnessObjective = 0.0;
            double accuracyObjective = 1.0;
            ...
            /* Weighted sum of objective functions */
            //objective = distance + lower_bound;
            objective =  (1-beta)*misc - beta*fairness_sample_robustness + lower_bound;
        } else {
            cout << "Mode was checked before, should never go here!" << endl;
            exit(-1);
        }
        ...
        if (objective < tree->min_objective()) {
            if(mode == 3 || mode == 5 || mode == 6) { // check if the constraint on fairness is satisfied
                // std::cout << "Mode = " << mode << ", minimum acceptable sample robustness = " << min_sample_robustness << endl;
                bool updateSol = false;
                if(mode == 3 || mode == 5){
                    if((1-unfairness) > min_fairness_acceptable) {
                       updateSol = true;                      
                    }
                    ...
                    if((1-unfairness) > min_fairness_acceptable){
                        ...
                        switch (fairness)
                        {
                            case 1: {
                                ...
                                total_solving_time+= duration.count();
                        ...
                        if(fairness_sample_robustness > min_sample_robustness){
                            updateSol = true;
                        }
                    }
                }
...
void bbound_loop(CacheTree* tree, 
    ...
    if (node_ordered.first) {
        ...
        evaluate_children(tree, node_ordered.first, node_ordered.second, not_captured, q, p, beta, fairness, maj_v, min_v, mode, useUnfairnessLB,
                        min_fairness_acceptable, forbidSensAttr, accuracyUpperBound, min_sample_robustness);
...
std::vector<unsigned long> bbound_end(CacheTree* tree, Queue* q, PermutationMap* p, bool early, rule_t* rules, rule_t* labels) {
    int verbosity = logger->getVerbosity();
    bool print_queue = 0;
    std::cout << "Sample robustness was in [" << min_robustness << ";" << max_robustness_not_one << " - " << max_robustness << "]." << std::endl;
    std::cout << "Average solving time was " << std::scientific  << total_solving_time/total_solver_calls << "]." << std::endl;
    if(debug) {
    ...
    // Debug for sample robustness
    /*cout << "Sample Robustness Scores were :" << endl;
    cout << "[";
    for (int i = 0; i < jaccard_distances.size(); i++) {
        cout << jaccard_distances[i] << ", ";
    }
    cout << "]" << endl;*/
    /*ofstream myfile;
    myfile.open ("vector-sample-robustness-scores.txt");
    myfile << "[";
    for (int i = 0; i < jaccard_distances.size(); i++) {
        myfile << jaccard_distances[i] << ", ";
    }
    myfile << "]" << endl;
    myfile.close();*/
    print_memo_info_sample_robustness_auditor();
