...
// julien: j'ai ajouté l'include suivant
#include <exception>

// julien: j'ai ajouté la classe d'exception suivante
struct ExceptionConstraintInconsistent : public std::exception {
   const char * what () const throw () {
      return "Problem in constraint propagation. Should start search again with another solver config!";
   }
};
