  ...
  if(parameters.checked) {
	...
	std::cerr << "\n@ p" << statistics.num_propagations ;
	  if(tmp_sol.size < k) {
	    ...
	    //exit(0); <- julien: j'ai commenté cette ligne
		//std::cout << "hello, you're in 1" << std::endl;
		throw ExceptionConstraintInconsistent(); // <- julien: j'ai ajouté cette ligne
	  } else {
		...
		//std::cout << "hello, you're in 2" << std::endl;
	    //exit(0); <- julien: j'ai commenté cette ligne
		throw ExceptionConstraintInconsistent(); // <- julien: j'ai ajouté cette ligne
	  }
