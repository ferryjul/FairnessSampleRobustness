...
from .sample_robustness_auditor_ortools_CP import audit_robustness_or_tools_cp
...
class SampleRobustnessAuditor():
    ...
    def audit_robustness(self, s_a, s_b, z_a, z_b, epsilon, config=0, debug=0, solver='Mistral'):
        ...
        if solver == 'Mistral':
            res = audit_sample_robustness(s_a, s_b, z_a, z_b, epsilon, debug, config, objective_lower_bound, objective_upper_bound)
        elif solver == 'OR-tools' and config == 0:
            #display=False
            #start = time.clock()
            res = audit_robustness_or_tools_cp(s_a, s_b, z_a, z_b, epsilon, debug, objective_lower_bound, objective_upper_bound)
        ...
        if status == 0: # found subset violating fairness
            unfairness = abs(((float)(s_a-x_a) /(float)(z_a-x_a-y_a ))  - 	 ((float)(s_b-x_b) /(float) (z_b-x_b-y_b) ))
