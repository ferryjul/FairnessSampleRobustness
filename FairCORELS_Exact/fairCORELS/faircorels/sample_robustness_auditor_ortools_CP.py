def audit_robustness_or_tools_cp(s_a, s_b, z_a, z_b, epsilon, debug, objective_lower_bound, objective_upper_bound):

    if s_a == 0 and s_b == 0:
        return -1, -1, -1, -1, -1, 1
    if s_a == z_a and s_b == z_b:
        return -1, -1, -1, -1, -1, 1
    ...
    while int(scaledEpsilon) != scaledEpsilon:
        scaler*=10
        scaledEpsilon = epsilon * scaler
    scaledEpsilon = int(scaledEpsilon)
    ...
    diff_abs_scaled = model.NewIntVar(0, max(s_a * z_b, s_b * z_a) * scaler, 'diff_abs_scaled')
    model.Add(diff == prod1 - prod2)
    model.AddAbsEquality(diff_abs, diff)
    model.Add(diff_abs_scaled == diff_abs* scaler)
    ...
    #print( z_a * z_b * scaledEpsilon)
    other_term_scaled  = model.NewIntVar(0, z_a * z_b * scaledEpsilon, 'other_term_scaled')
    model.Add(other_term_scaled == other_term * scaledEpsilon)
    #bool_for_or = model.NewIntVar(0, 1, 'disjunction_switch')
    #{model.Add(diff_abs)
    model.Add(other_term_scaled < diff_abs_scaled)
