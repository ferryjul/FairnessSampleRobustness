...
def fit_wrap_begin(np.ndarray[np.uint8_t, ndim=2] samples, 
             ...
             np.ndarray[np.uint8_t, ndim=2] min_vect, double accuracy_upper_bound, int max_calls, auditor_robustness_arg):
    global rules
    global labels_vecs
    global minor
    global n_rules
    global auditor_robustness
    global maj_vecs
    global min_vecs

    auditor_robustness = auditor_robustness_arg
...
def fit_wrap_loop(size_t max_nodes, double beta, int fairness,
                int mode, int useUnfairnessLB, double min_fairness_acceptable, int kBest, int restart, int initNBNodes, double geomReason,
                double min_sample_robustness_arg):
    ...
    cdef double min_sample_robustness = min_sample_robustness_arg
    # This is where the magic happens
    return (run_corels_loop(max_num_nodes, beta_val, fairness_metric, mode_val, useUnfairnessLB_val, min_fairness_acceptable_val, kBest_val, restart_val, initNBNodes_val, geomReason_val, min_sample_robustness) != -1)
...
# Below: adds for sample (Jaccard) robustness auditing/quantification

cdef extern from "src/corels/src/sample_robustness.cpp":
...
def audit_sample_robustness(int s_a, int s_b, int z_a, int z_b, float epsilon, int debug, int config, int objective_lower_bound, int objective_upper_bound):
    cdef int s_a_int = s_a
    cdef int z_a_int = z_a
    cdef int s_b_int = s_b
    cdef int z_b_int = z_b
    cdef int config_int = config
    cdef int debug_int = debug
    cdef int objective_lower_bound_int = objective_lower_bound
    cdef int objective_upper_bound_int = objective_upper_bound
    cdef float epsilon_int = epsilon
    cdef int* res = <int*>malloc(sizeof(int) * 6)
    score = quantify_robustness(s_a_int, s_b_int, z_a_int, z_b_int, epsilon_int, config_int, debug_int, objective_lower_bound_int, objective_upper_bound_int, res)
    if res[5] == 0:
        if score != res[4]:
            print("internal error while getting sample robustness audit results, score mismatch between ", score, " and ", res[4], ". Exiting")
            exit()
    ret=[res[0], res[1], res[2], res[3], res[4],res[5]]
    free(res)
    return ret

# ----- Everything below is for sample robustness audit
from .sample_robustness import SampleRobustnessAuditor
# auditor_robustness = SampleRobustnessAuditor()

# declare your function as cdef
#cdef public void init_sample_robustness_auditor(int memoisation):
#    #print 'doing some python here', memoisation
#    global auditor_robustness
#    auditor_robustness = SampleRobustnessAuditor(memoisation)
#    print("Initialized sample robustness auditor")

cdef public double compute_sample_robustness(int s_a, int s_b, int z_a, int z_b, double epsilon, int config, int debug, int solver):
    #print 'doing some python here', memoisation
    global auditor_robustness
    if solver == 0:
        solver_arg = 'Mistral'
        ...
        solver_arg = 'OR-tools'
    audit_result = auditor_robustness.audit_robustness(s_a, s_b, z_a, z_b, epsilon, config, debug, solver_arg)
    #print("Measured sample robustness is ", audit_result.jaccard_dist)
    return audit_result.jaccard_dist

cdef public void print_memo_info_sample_robustness_auditor():
    #print 'doing some python here', memoisation
    global auditor_robustness
    auditor_robustness.print_memo_statistics()