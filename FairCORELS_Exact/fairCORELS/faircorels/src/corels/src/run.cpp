...
{
    // Initialize sample robustness auditor object (useful for memoisation)
    // init_sample_robustness_auditor(2);
...
int run_corels_loop(size_t max_num_nodes, double beta, int fairness, int mode, bool useUnfairnessLB,
    ...
    // Check arguments
    if(mode < 1 || mode > 6) {
        printf("Error : mode should be in {1, 2, 3, 4, 5}\n");
        exit(-1);
    }
    if(kBest < 0) {
    ...
    if(restart == 1){ // Perform geometric restart
        ...
        if((g_tree->num_nodes() < currLimit) && !g_queue->empty()) {
            bbound_loop(g_tree, g_queue, g_pmap, beta, fairness, Gmaj_vect, Gmin_vect, mode, useUnfairnessLB,
                            min_fairness_acceptable, kBest, forbidSensAttr, accUpperBound, 0.0); 
            return 0;
        } else {
        ...
        if((g_tree->num_nodes() < currLimit) && !g_queue->empty()) {
            ...
            if((g_tree->num_nodes() < max_num_nodes) && !g_queue->empty() && (callsNB > 0)) {
                bbound_loop(g_tree, g_queue, g_pmap, beta, fairness, Gmaj_vect, Gmin_vect, mode, useUnfairnessLB,
                                min_fairness_acceptable, kBest, forbidSensAttr, accUpperBound, min_sample_robustness);
                callsNB--;
                if(callsNB == 0){
                    if(debugRun){
