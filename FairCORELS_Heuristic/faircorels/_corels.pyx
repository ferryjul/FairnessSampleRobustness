...
def fit_wrap_begin(np.ndarray[np.uint8_t, ndim=2] samples, 
             ...
             np.ndarray[np.uint8_t, ndim=2] min_vect, double accuracy_upper_bound, int max_calls, int nb_masks):
    global rules
    global labels_vecs
    global minor
    global n_rules
    ...
    cdef int rb = run_corels_begin(c, verbosity, policy, map_type, ablation, calculate_size,
                   ...
                   forbidSensAttr_val, maj_vecs, n_maj_vecs, min_vecs, n_min_vecs, 
                   accuracy_upper_bound, max_calls, nb_masks)
