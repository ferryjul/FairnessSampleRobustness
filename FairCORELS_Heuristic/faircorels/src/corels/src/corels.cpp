...
confusion_matrix_groups compute_confusion_matrix(VECTOR parent_prefix_predictions,
    ...
    //int nbMask = count_ones_vector(mask, tree->nsamples());
    //printf("Mask vector : captures %d/%d instances.\n", nbMask, tree->nsamples());
    int nsamples = tree->nsamples();
    int pm, num_not_captured;
    VECTOR preds_prefix, not_captured;
    ...
    // apply mask
    rule_vand(TP, TP, mask, nsamples, &pm);
    rule_vand(FP, FP, mask, nsamples, &pm);
    rule_vand(FN, FN, mask, nsamples, &pm);
    rule_vand(TN, TN, mask, nsamples, &pm);
...
void evaluate_children(CacheTree* tree, 
    ...
    if(firstCall){
        firstCall = false;
        if(min_fairness_acceptable < 1.0){ // seuil is the max fairness violation accross DRO subsets, it must be scaled depending on min_fairness_acceptable
            while((seuil/(1-min_fairness_acceptable))>1.0001){
                double oldSeuil = seuil;
                seuil /= 10;
                //printf("seuil updated : %lf -> %lf\n", oldSeuil, seuil);
            }
        }  
        //printf("Seuil will be %lf\n", seuil);
        VECTOR captured_it;
    ...
    for (i = 1; i < nrules; i++) {
        ...
        // Unfairness for entire training set
        ...
        //printf("reg term = %lf\n", max(0.0, max_sp - (1-min_fairness_acceptable)));

        // Fairness metrics for entire training set
        fairness_metrics fm = compute_fairness_metrics(cmg);
        
        //test
        /*double sum_unf = 0;
        double sum_unf_4 = 0;
        for(int mask_index = 0 ; mask_index < nb_masks ; mask_index++){
            sum_unf += fms[mask_index].statistical_parity;
            sum_unf_4 += fms[mask_index].equal_opportunity;
        }
        sum_unf /= nb_masks;
        sum_unf_4 /= nb_masks;
        if(fabs(sum_unf - fm.statistical_parity) > 0.00001)
            printf("Error : sum SP = %lf, tr SP = %lf\n", sum_unf, fm.statistical_parity);
        if(fabs(sum_unf_4 - fm.equal_opportunity) > 0.00001)
            printf("Error : sum EO = %lf, tr EO = %lf\n", sum_unf_4, fm.equal_opportunity);*/
        ...
        cout << "unfairness is " << unfairness << endl;
        if(useUnfairnessLB) {
        ...
        if(mode == 2) { // Max fairness
            ...
            double misc = (double)(num_not_captured - default_correct) / nsamples;
            if(fairness == 1)
                objective =  misc + lower_bound;// + max(0.0, max_sp - (1-min_fairness_acceptable)); // ou max_eo
            else if(fairness == 4)
                objective =  misc + lower_bound; // + max(0.0, max_eo - (1-min_fairness_acceptable)); // ou max_eo
            else
        ...
        if (objective < tree->min_objective()) {
            if(mode == 3) { // if mode 3 we check if the constraint on fairness is satisfied
                if(((1-unfairness) > min_fairness_acceptable)) { // && (max(0.0, max_sp - (1-min_fairness_acceptable))<0.001)
                // Fairness metrics for DRO
                // Unfairness for DRO
                confusion_matrix_groups cmgs[nb_masks];
                fairness_metrics fms[nb_masks];
                bool stillPossible = true;
                for(int mask_index = 0 ; (mask_index < nb_masks) && stillPossible ; mask_index++){
                    cmgs[mask_index] = compute_confusion_matrix(preds_prefix, tree, parent_not_captured, captured,
                                                                                        maj_v, min_v, prediction, default_prediction,
                                                                                        masks[mask_index]);
                    fms[mask_index] = compute_fairness_metrics(cmgs[mask_index]);

                    double max_val = 0;
                    if(fairness == 1){
                        max_val = max(0.0, fms[mask_index].statistical_parity - (1-min_fairness_acceptable));
                    } else if(fairness == 4) {
                        max_val = max(0.0, fms[mask_index].equal_opportunity - (1-min_fairness_acceptable));
                    } else if(fairness == 2) {
                        max_val = max(0.0, fms[mask_index].predictive_parity - (1-min_fairness_acceptable));
                    } else if(fairness == 3) {
                        max_val = max(0.0, fms[mask_index].predictive_equality - (1-min_fairness_acceptable));
                    } else if(fairness == 5) {
                        max_val = max(0.0, fms[mask_index].equalized_odds - (1-min_fairness_acceptable));
                    } else if(fairness == 6) {
                        max_val = max(0.0, fms[mask_index].cond_use_acc_equality - (1-min_fairness_acceptable));
                    }
                    if(max_val >= seuil){
                        stillPossible = false;
                    }
                }
                // Find worst case unfairness
                //5 : equalized odds, 6 : conditional use accuracy equality
                /*double max_sp = 0; // 1
                double max_eo = 0; // 4
                double max_pp = 0; // 2
                double max_pe = 0; // 3
                double max_eodds = 0; // 5
                double max_cuae = 0; // 6
                fairness_metrics fms[nb_masks];
                for(int mask_index = 0 ; mask_index < nb_masks ; mask_index++){
                    fms[mask_index] = compute_fairness_metrics(cmgs[mask_index]);
                    if(fms[mask_index].statistical_parity > max_sp){
                        max_sp = fms[mask_index].statistical_parity;
                    }
                    if(fms[mask_index].equal_opportunity > max_eo){
                        max_eo = fms[mask_index].equal_opportunity;
                    }
                    if(fms[mask_index].predictive_parity > max_pp){
                        max_pp = fms[mask_index].predictive_parity;
                    }
                    if(fms[mask_index].predictive_equality > max_pe){
                        max_pe = fms[mask_index].predictive_equality;
                    }
                    if(fms[mask_index].equalized_odds > max_eodds){
                        max_eodds = fms[mask_index].equalized_odds;
                    }
                    if(fms[mask_index].cond_use_acc_equality > max_cuae){
                        max_cuae = fms[mask_index].cond_use_acc_equality;
                    }
                }
                
                double max_val = 0;
                if(fairness == 1){
                    max_val = max(0.0, max_sp - (1-min_fairness_acceptable));
                } else if(fairness == 4) {
                    max_val = max(0.0, max_eo - (1-min_fairness_acceptable));
                } else if(fairness == 2) {
                    max_val = max(0.0, max_pp - (1-min_fairness_acceptable));
                } else if(fairness == 3) {
                    max_val = max(0.0, max_pe - (1-min_fairness_acceptable));
                } else if(fairness == 5) {
                    max_val = max(0.0, max_eodds - (1-min_fairness_acceptable));
                } else if(fairness == 6) {
                    max_val = max(0.0, max_cuae - (1-min_fairness_acceptable));
                }*/
                //if((max_val < seuil)){
                if(stillPossible){
 ...
 */
void bbound_begin(CacheTree* tree, Queue* q, int seed, int nb_masks_arg) {
    start = timestamp();
    num_iter = 0;
    ...
    // DRO masks computation
    nb_masks = nb_masks_arg;
    int nb_samples = tree->nsamples();
    int nones;
    int chk;
    char * random_seq = (char*) malloc(sizeof(char)*(nb_samples+1));
    //printf("Initializing %d masks.\n", nb_masks);
    srand(seed*2);
    for(int i = 0 ; i < nb_masks ; i++){
        rule_vinit(tree->nsamples(), &masks[i]);
        nb_samples = tree->nsamples();
        // for debug only : init with ones
        // rule_vor(masks[i], tree->label(0).truthtable, tree->label(1).truthtable, tree->nsamples(),&nb_samples);
        for(int j = 0 ; j < tree->nsamples() ; j ++){
            if(rand() % 2){
                random_seq[j] = '0';
            } else {
                random_seq[j] = '1';
        ...
        random_seq[nb_samples] = '\0';
        chk = ascii_to_vector(random_seq, nb_samples+1, &nb_samples, &nones, &masks[i]);
        if(chk){
            printf("Problem while creating random vectors. chk = %d.\n", chk);
        }
    }
    free(random_seq);
    //printf("Initialized %d masks.\n", nb_masks);
    //for(int i = 0 ; i < nb_masks ; i++){
    //    printf("vector %d captures %d/%d instances.\n", i, count_ones_vector(masks[i], tree->nsamples()), tree->nsamples());
    //}
}
