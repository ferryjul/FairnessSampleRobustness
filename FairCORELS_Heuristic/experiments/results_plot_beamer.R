...
main_plot_test <- function(dataset, metric, minX, maxX, minY, maxY) {
  ...
  output_file             <- sprintf("./graphs/%s_test_%s_nomasks.pdf", dataset, metric)
  ...
  df <- df %>%
    filter(masks == 'no mask')
  
  pp <- ggplot() + 
    
    geom_line(data=df, aes(x=error_test, y=unf_test), colour="#a83546",  size=line_size, linetype = "dashed", alpha=alpha_line) + 
    
    geom_point(data=df, aes(x=error_test, y=unf_test, shape=masks), colour="#a83546", size=point_size, alpha=alpha_point) + 
...
main_plot_train <- function(dataset, metric, minX, maxX, minY, maxY) {
  ...
  output_file             <- sprintf("./graphs/%s_train_%s_nomasks.pdf", dataset, metric)
  df                      <- read.csv(input_file, header=T)
  ...
  pp <- ggplot() + 
    ...
    geom_line(data=df, aes(x=error_train, y=unf_train), colour="#a83546", size=line_size, linetype = "dashed", alpha=alpha_line) + 
    
    geom_point(data=df, aes(x=error_train, y=unf_train, shape=masks), colour="#a83546", size=point_size, alpha=alpha_point) + 
...
fairness_violation_plot <- function(dataset, metric) {
  ...
  output_file             <- sprintf("./graphs/%s_fairness_violation_%s_nomasks.pdf", dataset, metric)
  df                      <- read.csv(input_file, header=T)
  ...
  pp <- ggplot() + 
    ...
    geom_point(data=df, aes(x=unf_train, y=unf_test, shape=masks), colour="#a83546", size=point_size, alpha=alpha_point) + 
...
plot_leggend <- function (dataset, metric){
  ...
  input_file              <- sprintf("./results_merged/%s_test_%s.csv", dataset, metric)
  legend_file             <- sprintf("./graphs/legend_nomasks.pdf")
  df                      <- read.csv(input_file, header=T)
