...
X, y, features, prediction = load_from_csv("./data/compas_fullRules_gen.csv")#("./data/adult_full.csv") # Load the dataset
...
parser.add_argument('--filteringMode', type=int, default=1, help='filtering mode (0 no pruning, 1 lazy, 2 eager)')
...
parser.add_argument('--filteringTech', type=str, default='CP', help='filtering tech (CP or MIP)')
parser.add_argument('--filteringUB', type=int, default=0, help='compute filtering new ub (0: no, 1: yes)')

args = parser.parse_args()

epsilon = args.epsilon
filteringTech = args.filteringTech
bools = [False, True]
filteringUB = bools[args.filteringUB]
...
def oneFold(foldIndex, X_fold_data): # This part could be multithreaded for better performance
    ...
    # Train it
    clf.fit(X_train_unprotected, y_train, features=features[2:], prediction_name="(recidivism:yes)", nb_masks = n_masks)#, time_limit = 10)# max_evals=100000) # time_limit=8100, 
